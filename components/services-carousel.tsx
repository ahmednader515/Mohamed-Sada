...
export default function ServicesCarousel({ services, title }: ServicesCarouselProps) {
  ...
  return (
    <div className="w-full py-10">
      <h2 className="text-2xl font-bold text-center mb-6 text-[#8B0620]">{title}</h2>
      ...
      <div className="relative">
        <div className="swiper-container relative" style={{ minHeight: '500px', paddingBottom: '60px' }}>
          ...
          <div className="hidden md:block">
            ...
            >
              <ChevronLeft className="h-6 w-6 text-[#8B0620]" />
            </button>
            <button 
              onClick={goToPrev}
            ...
            >
              <ChevronRight className="h-6 w-6 text-[#8B0620]" />
            </button>
          </div>
          <Swiper
            onSwiper={(swiper) => {
          ...
          <SwiperSlide key={index}>
            <div
              className={`rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 ${
                index === 1 ? 'bg-[#8B0620] text-white transform scale-105' : 'bg-white text-[#8B0620]'
              }`}
            >
              {/* Service Icon and Title */}
              <div className="text-center mb-6">
                <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
                  index === 1 ? 'bg-white/20' : 'bg-[#8B0620]/10'
                }`}>
                  <service.icon className={`h-8 w-8 ${index === 1 ? 'text-white' : 'text-[#8B0620]'}`} />
                </div>
              ...
              <div className="space-y-3 mb-8">
                <div className="flex items-center">
                  <CheckCircle className={`h-5 w-5 mr-3 ${index === 1 ? 'text-white' : 'text-[#8B0620]'}`} />
              ...
              <Button 
                className={`w-full py-3 text-lg font-semibold rounded-xl transition-all duration-300 ${
                  index === 1 
                    ? 'bg-white text-[#8B0620] hover:bg-gray-100' 
                    : 'bg-[#8B0620] text-white hover:bg-[#8B0620]/90'
                }`}
              >
                احجز الآن
              </Button>
